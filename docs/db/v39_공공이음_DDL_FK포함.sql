...
-- SHOW CREATE TABLE + 실데이터로 그대로 다시 만든 v36 스냅샷을 바탕으로,
-- v37~v39 변경분을 2026-07-30 기준 SHOW CREATE TABLE 재조회 결과로 반영해
-- 갱신한 것이다.
--
...
--      이 문서 작성 중 라이브 DB를 SHOW CREATE TABLE로 재조회해 다음 불일치를
--      발견/수정함(라이브가 항상 진실의 원천): (1) announcement 테이블에
--      department_code 컬럼과 fk_ann_department FK(department 참조, ON
--      DELETE SET NULL)가 이미 있었음 - v38 작업분에 포함돼 있었으나 최초
--      태스크 문서에는 빠져 있었음. (2) notification.type 컬럼 코멘트가
--      "알림 종류 - CHAT_MESSAGE | ANNOUNCEMENT"로 이미 갱신되어 있었음.
--      (3) chat_room_id/announcement_id처럼 ALTER TABLE로 나중에 추가한
--      FK들은 별도 인덱스명을 지정하지 않아, MySQL이 인덱스명을 제약조건명과
--      동일하게 자동 부여함(fk_project_chat_room, fk_noti_announcement,
--      fk_ann_department) - 문서의 idx_* 명명 규칙 대신 실제 라이브 인덱스명을
--      그대로 반영함.
--
-- 실제 운영 DB에는 지금 common_code -> common_code_group 사이의 FK
-- (fk_commoncode_group), v36에서 새로 생성한 채팅/알림 테이블 5개의 FK,
-- v38에서 팀원이 추가한 announcement 테이블의 FK 3개(fk_ann_created_by,
-- fk_ann_updated_by, fk_ann_department)와 notification.announcement_id의
-- fk_noti_announcement, 그리고 v39에서 우리가 추가한 project.chat_room_id의
-- fk_project_chat_room만 실제로 걸려 있고, 나머지 기존 관계는 전부 일반
-- 인덱스뿐이다. 이 파일의 그 나머지 관계에는 FK 제약을 추가한 "있어야 할
-- 모습" 버전이다.
-- ============================================================
...
CREATE TABLE project (
  ...
  chat_room_id         int          DEFAULT NULL COMMENT '자동 생성된 사업 협업 채팅방',
  PRIMARY KEY (project_id),
  KEY idx_project_task (task_id),
  KEY idx_project_department (department_code),
  KEY fk_project_chat_room (chat_room_id),
...
CREATE TABLE announcement (
  ...
  created_at      datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at      datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  department_code varchar(10)  DEFAULT NULL COMMENT '작성 부서 (NULL=전체 공개)',
  PRIMARY KEY (announcement_id),
  KEY idx_ann_created_by (created_by),
  KEY idx_ann_updated_by (updated_by),
  KEY fk_ann_department (department_code),
  CONSTRAINT fk_ann_created_by FOREIGN KEY (created_by) REFERENCES `user` (user_id) ON DELETE RESTRICT,
  CONSTRAINT fk_ann_department FOREIGN KEY (department_code) REFERENCES department (department_code) ON DELETE SET NULL,
...
CREATE TABLE notification (
  ...
  type             varchar(20)  NOT NULL COMMENT '알림 종류 - CHAT_MESSAGE | ANNOUNCEMENT',
  ...
  KEY fk_noti_announcement (announcement_id),
